...
개발 및 코드 기여 시 에이전트 지침인 [.agents/AGENTS.md](file:///Users/east/work/lumina-jump/.agents/AGENTS.md)을 엄격히 준수하며 일관된 코딩 스타일을 유지해야 합니다.
...
- 세부적인 버전업 변경 기준(SemVer) 및 빌드/푸시 상세 프로세스는 AI 에이전트 개발 지침인 [.agents/AGENTS.md](file:///Users/east/work/lumina-jump/.agents/AGENTS.md)의 **4.1 릴리즈 및 버전 관리 규칙**을 참고하시기 바랍니다.

---

## 🔄 최근 업데이트 내역 (Changelog)

### [2026-07-07] - 프로젝트 명칭 일관성 및 오프라인 동기화 개선
- **프로젝트 명명 규칙 일관성 적용**: 로컬스토리지에 저장되는 재화 데이터의 키 값(`flappy-candy-coins`)을 새로운 프로젝트명에 걸맞게 `lumina_crystals`로 변경했습니다. 기존 플레이어의 코인이 초기화되지 않도록 하위 호환 마이그레이션 로직을 반영했습니다.
- **오프라인 랭킹 자동 동기화 고도화**: 오프라인 상태에서 새로운 최고 점수를 등록한 후 다시 온라인 상태로 복귀했을 때, 서버 리더보드에 누락되어 있던 로컬의 최고 기록을 백그라운드에서 감지하여 자동으로 서버에 동기화(`syncOfflineScore`)해 주는 루틴을 추가하여 데이터 정합성을 극대화했습니다.
